...
public abstract class AbsActivity extends AppCompatActivity implements BaseView {
  ...
  MyPermissionUtils.OnResponsePermissionListener mPermissionResponseListener;
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    ActivityStackManager.getInstance()
                        .addActivity(new WeakReference(this));
    statusBarSetting();
    compositeDisposable = new CompositeDisposable();
    mLoadingContainer = LayoutInflater.from(this)
                                      .inflate(R.layout.invoice_loading_layout, null);
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    for (int index = 0; index < fm.getFragments()
                                  .size(); index++) {
      Fragment fragment = fm.getFragments()
                            .get(index); //找到第一层Fragment
      if (fragment != null) {
  ...
  private void handleResult(Fragment fragment, int requestCode, int resultCode, Intent data) {
    ...
    List<Fragment> childFragment = fragment.getChildFragmentManager()
                                           .getFragments();
   ...
   *
   * @param permissionResponseListener the permission response listener
   */
