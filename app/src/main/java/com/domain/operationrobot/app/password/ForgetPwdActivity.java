...
public class ForgetPwdActivity extends AbsActivity {
    ...
    private void commit(String phone, String pwd, String code) {
        showProgress();
        Observable<BaseEntry<String>> baseEntryObservable = RemoteMode.getInstance().foegetPwd(phone, pwd, code);
        baseEntryObservable.subscribe(new BaseObserver<String>(compositeDisposable) {
            @Override
    ...
    public void sendCode(String accountPhone) {
        showProgress();
        timer.start();
        RemoteMode.getInstance().sendCode(accountPhone).subscribe(new BaseObserver<String>(compositeDisposable) {
            @Override
            public void onError(BaseException e) {
                hideProgress();
                showToast(e.getMsg());
