...
public class AccountPresenterImpl extends AccountContract.AccountPresenter<AccountContract.AccountView> {
    ...
    public void sendCode(String accountPhone) {
        showProgress();
        timer.start();
        RemoteMode.getInstance().sendCode(accountPhone).subscribe(new BaseObserver<String>(getCompositeDisposable()) {
            @Override
    ...
    public void commit(String phone, String pwd, String code) {
        showProgress();
        RemoteMode.getInstance().createAccount(phone, pwd, code).subscribe(new BaseObserver<User>(getCompositeDisposable()) {
            @Override
            public void onError(BaseException e) {
                if (mView == null) return;
                hideProgress();
            ...
            @Override
            public void onSuss(BaseEntry<User> userBaseEntry) {
                if (mView == null) return;
                hideProgress();
                mView.createSuss(userBaseEntry.getResultData());
            }
        });
    }
